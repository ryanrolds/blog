...
The above request's headers include `Cache-Control: max-age=0` and `Pragma: no-cache` (legacy). The directive  `max-age=0` tells the server (and any intermediate caches, like a CDN or CloudFlare) to not use a cached copy and to check the origin (the blog server). `Pragma` is a legacy HTTP/1.0 header and is replaced by HTTP/1.1's `Cache-Control`, it's still useful to set as not everyone keeps their services up-to-date (but seriously we've had HTTP/1.1 for nearly two decades, WTF?).

The response's headers contain `Cache-Control: public, must-revalidate` and `ETag: e66d665cd7a6d67ca6112b21c6351c1c`. These headers give the browser (and intermediate caches) the rules around caching this URI. A response's `Cache-Control` header may contain a few directives. The most common are `public` (should cache), `private` (should not be put in a shared cache), `no-cache` (do not cache), `max-age=<seconds>`, and `must-revalidate`. There are quite a view other directives, check out MDN's [documentation](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) for `Cache-Control`. 

Keep in mind that `max-age=<seconds>` can have a different meaning depending on the context. If it's in the Request Headers it limits how old of a cache entry intermediate caches are allowed respond with. If the intermediate cache's entry is recent enough then the request never makes it to the origin server. If it's too old then the intermediate cache checks with the origin, possibly pulling a fresher response. If the directive is in the Response Headers, it defines a period in which the asset an can simply be served from the browser's cache, no request needed. Once this period passes then it must be refreshed/revalidated.
...
Things that are not volatile and are used often deserve aggressive caching. Things that change often shouldn't be cached or at least should be revalidated often. Sensitive data should not be put in a shared cached (`private` directive). `PUT`, `POST`, and `DELETE` requests are not safe to cache. Things you don't want cached should have the `no-cache` directive.

One problem that comes up is needing to bust the cache. This problem usually manifests as someone deployed a bug fix and they don't want anyone to continue using the old/buggy version. Your only option is to deploy code that uses a new URI for the affected asset. Some projects put versions in the filename/path and others append a version or hash to the query string. I usually go with the query string hash, because who wants to version every asset? The code for storing and managing a version for every asset is non-trivial. Using some kind of hash (MD5, SHA, etc...) avoids having to store asset versions because the hash is intrinsic to the asset's data, changing the asset is changing the MD5.
...
First, let's define the strategy we will use. All responses will have an `ETag` header that contains the MD5 sum of the Response Body. Responses containing HTML will have the `must-revalidate` directive, that way we can be sure users always have a fresh version but we can also avoid having to resend what they already have.
